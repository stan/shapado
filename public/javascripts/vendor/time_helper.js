(function($) {
  $.timeformat = function(timestamp, format) {
    if (timestamp instanceof Date) {
      ...
      return strftime($.timeformat.parser.call(this, timestamp,format));
    } else {
      return strftime($.timeformat.datetime(timestamp,format));
    }
  };
  var $t = $.timeformat;
  ...
  $.extend($.timeformat, {
    settings: {
      ...
      strings: {
        ...
        day_mames: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"],
    ...
    distanceOfTimeInWords: function(from_time, to_time, include_seconds) {
      ...
      opts = {scope: 'distance_in_words', data: this.settings.strings};
      ...
      while(regexp.test(format)) {
        ...
        if(a) {
          var result = '';
            switch(a) {
              case '%A': {
                result = $l.dayNames[date.getDay()];
                break;
              }
              case '%a': {
                result = $l.abbrDayNames[date.getDay()];
                break;
              }
              case '%d': {
                result = date.getDate();
              ...
              case '%B': {
                result = $l.monthNames[date.getMonth()];
                break;
              }
              case '%b': {
                result = $l.abbrMonthNames[date.getMonth()];
                break;
              }
              case '%m': {
    ...
    parser: function() {
      return Date['parse'] ? Date['parse'] : this.parse
    },
    ...
    if($t.settings.timeago) {
      ...
      self.each(function() {
        ...
        if(format){
          ...
          $(this).text(strftime($t.datetime(element), format));
        }
      });
    }
  ...
  function refresh() {
    ...
    if (element.attr('datetime')) {
      element.text($t.settings.strings.ago.replace('%{time}', distanceOfTimeInWords($t.parse(element.attr('datetime')))));
    }
    return this;
  }

  function strftime(date, format) {
    return $t.strftime(date, format);
  }

  function distanceOfTimeInWords(date) {
    return $t.distanceOfTimeInWords(date);
  }
}(jQuery));
